...
<header>
<blockquote>
	<img width="90" src="image/logo.png">
...
if(isset($_SESSION['id'])){
	...
	$sql = "SELECT customer.CustomerName, customer.CustomerGender, customer.CustomerAddress, customer.CustomerEmail, customer.CustomerPhone, food.FoodTitle, food.Price, food.Image, `order`.`DatePurchase`, `order`.`Quantity`, `order`.`TotalPrice`
...
<?php
	...
	while($row = $result->fetch_assoc()){
    	...
    	echo $row['FoodTitle']."<br>RM".$row['Price']."<br>";
...
$nameErr = $emailErr = $genderErr = $addressErr = $contactErr = "";
$name = $email = $gender = $address = $contact = "";
$cID;
						...
						if(!preg_match("/^[0-9 -]*$/", $contact)){
							...
							if (empty($_POST["gender"])){
								...
								if (empty($_POST["address"])){
									...
									$sql = "SELECT CustomerID from customer WHERE CustomerName = '".$name."'";
									$result = $conn->query($sql);
									while($row = $result->fetch_assoc()){
										$cID = $row['CustomerID'];
									}
